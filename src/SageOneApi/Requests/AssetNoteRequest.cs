...
namespace SageOneApi.Requests
{
	public class AssetNoteRequest : RequestBase, IAssetNoteRequest
	{
		...
		{
			var url = string.Format("AssetNote/Get?apikey={0}&companyid={1}", _apiKey, _companyId);

			if (!string.IsNullOrEmpty(filter))
				url = string.Format("AssetNote/Get?apikey={0}&companyid={1}&$filter={2}", _apiKey, _companyId, filter);
		...
		{
			var url = string.Format("AssetNote/Save?apikey={0}&companyid={1}", _apiKey, _companyId);
		...
		{
			var url = string.Format("AssetNote/Delete/{0}?apikey={1}&companyid={2}", id, _apiKey, _companyId);
